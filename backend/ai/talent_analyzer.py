...
class TalentAnalyzer:
    ...
    def analyze(self, cv_text: str) -> dict:
        prompt = f"""
You are the Talent Profile Analyzer for AI-UNER, an AI-powered recruitment platform.

TASK: Extract structured information from the following CV text.

CV TEXT:
{cv_text}

IMPORTANT RULES:
1. Extract ONLY information that is clearly stated in the CV
2. DO NOT invent or assume any information
3. Use null for missing information
4. Skills must be a list of strings
5. Experience must be a list of objects
6. duration_months must be a number or null
7. All arrays must be valid JSON arrays
